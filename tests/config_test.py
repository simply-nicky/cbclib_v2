...
class TestCPUConfig():
    ...
    def test_effective_num_threads_uses_config_in_main_thread(self):
        get_cpu_config().num_threads = 4

        assert get_cpu_config().effective_num_threads() == 4

    def test_effective_num_threads_uses_one_thread_in_pool_worker(self):
        get_cpu_config().num_threads = 4
        set_cpu_pool_worker(True)

        assert get_cpu_config().effective_num_threads() == 1

    def test_reset_cpu_config_preserves_pool_worker_context(self):
        get_cpu_config().num_threads = 4
        set_cpu_pool_worker(True)
