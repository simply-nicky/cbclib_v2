...
template <size_t N, typename T, typename Pt, typename I = std::remove_cvref_t<Pt>::value_type,
    typename = std::enable_if_t<
        std::is_base_of_v<PointND<I, N>, std::remove_cvref_t<Pt>> && std::is_integral_v<I>
    >
>
PixelND<T, N> make_pixel(Pt && point, const array<T> & data)
{
...
public:
    ...
    PixelsND(const PointSetND<N> & points, const array<T> & data)
    {
        for (auto && pt : points)
        {
            ...
            {
                m_pset.emplace_hint(m_pset.end(), make_pixel<N>(std::forward<decltype(pt)>(pt), data));
            }
        }
        m_mnt = m_pset;
    }
